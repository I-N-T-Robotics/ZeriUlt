...
public class LimelightVision extends SubsystemBase {
  ...
  // Rejection reason counters — reset each loop so dashboard shows current state
  private int rejectedAngularVelocity = 0;
  private int rejectedNoTags = 0;
  private int rejectedAreaThreshold = 0;
  ...
  public boolean rejectUpdate(PoseEstimate estimate, AngularVelocity angularVelocity) {
    ...
    // Single tag but area below threshold
    rejectedAreaThreshold++;
    return true;
  }

  public void setCurrentEstimates(AngularVelocity currentAngularVelocity) {
    // Reset counters each call so dashboard reflects current loop only
    rejectedAngularVelocity = 0;
    rejectedNoTags = 0;
    rejectedAreaThreshold = 0;
    accepted = 0;
    ...
    for (int i = 0; i < Cameras.LimelightCameras.length; i++) {
      ...
      if (estimate != null) {
        SmartDashboard.putNumber("Vision/" + name + "/tagCount", estimate.tagCount);
        SmartDashboard.putNumber("Vision/" + name + "/avgTagArea", estimate.avgTagArea);
        SmartDashboard.putNumber("Vision/" + name + "/avgTagDist", estimate.avgTagDist);
        SmartDashboard.putNumber("Vision/" + name + "/poseX", estimate.pose.getX());
        SmartDashboard.putNumber("Vision/" + name + "/poseY", estimate.pose.getY());
        SmartDashboard.putNumber("Vision/" + name + "/poseRot", estimate.pose.getRotation().getDegrees());
        SmartDashboard.putNumber("Vision/" + name + "/timestamp", estimate.timestampSeconds);
        SmartDashboard.putBoolean("Vision/" + name + "/estimateNull", false);
      } else {
        SmartDashboard.putBoolean("Vision/" + name + "/estimateNull", true);
        SmartDashboard.putNumber("Vision/" + name + "/tagCount", 0);
      }

      if (estimate != null && !rejectUpdate(estimate, currentAngularVelocity)) {
        lastEstimates[i] = estimate;
        estimateUpdated[i] = true;
        SmartDashboard.putBoolean("Vision/" + name + "/accepted", true);
      } else {
        SmartDashboard.putBoolean("Vision/" + name + "/accepted", false);
      }
    }

    // Global rejection reason summary
    SmartDashboard.putNumber("Vision/rejectedAngularVelocity", rejectedAngularVelocity);
    SmartDashboard.putNumber("Vision/rejectedNoTags", rejectedNoTags);
    SmartDashboard.putNumber("Vision/rejectedAreaThreshold", rejectedAreaThreshold);
  ...
  public Optional<PoseEstimate> updatePoseEstimates(AngularVelocity currentAngularVelocity) {
    ...
    // Log whether we actually sent a measurement to the drivetrain this loop
    SmartDashboard.putBoolean("Vision/sentMeasurement", best != null);
    if (best != null) {
      SmartDashboard.putNumber("Vision/sentPoseX", best.pose.getX());
      SmartDashboard.putNumber("Vision/sentPoseY", best.pose.getY());
      SmartDashboard.putNumber("Vision/sentPoseRot", best.pose.getRotation().getDegrees());
    }
